...
mutable struct CR3BPIndirectWithSTMParams
...
function cr3bpEomIndirectWithSTM!(du::AbstractVector, u::AbstractVector,
    ...
    @inbounds begin
        ...
        GVec = cr3bpEomIndirect!(view(du, 1:14), view(u, 1:14), p, t)
...
function cr3bpEOMJac!(jac, u, p, GVec)
    ...
    dG11dr          = @SVector [(1 - μ)*(μ + x[1])*(9*invr15 - 15*(x[1] + μ)^2*invr17) +
                    μ*(μ + x[1] - 1)*(9*invr25 - 15*(x[1] + μ - 1)^2*invr27),
                    (1 - μ)*x[2]*(3*invr15 - 15*(x[1] + μ)^2*invr17) +
                    μ*x[2]*(3*invr25 - 15*(x[1] + μ - 1)^2*invr27), 
                    (1 - μ)*x[3]*(3*invr15 - 15*(x[1] + μ)^2*invr17) + 
                    μ*x[3]*(3*invr25 - 15*(x[1] + μ - 1)^2*invr27)];
                    
    dG12dr          = @SVector [x[2]*(1 - μ)*(3*invr15 - 15*(x[1] + μ)^2*invr17) + 
                    x[2]*μ*(3*invr25 - 15*(x[1] + μ - 1)^2*invr27), 
                    (1 - μ)*(x[1] + μ)*(3*invr15 - 15*x[2]^2*invr17) + 
                    μ*(x[1] + μ - 1)*(3*invr25 - 15*x[2]^2*invr27), 
                    -15*(1 - μ)*(x[1] + μ)*x[2]*x[3]*invr17 - 
                        15*μ*(x[1] + μ - 1)*x[2]*x[3]*invr27];

    dG13dr          = @SVector [x[3]*(1 - μ)*(3*invr15 - 15*(x[1] + μ)^2*invr17) + 
                    x[3]*μ*(3*invr25 - 15*(x[1] + μ - 1)^2*invr27), 
                    -15*(1 - μ)*(x[1] + μ)*x[2]*x[3]*invr17 - 
                    15*μ*(x[1] + μ - 1)*x[2]*x[3]*invr27, 
                    (1 - μ)*(x[1] + μ)*(3*invr15 - 15*x[3]^2*invr17) + 
                    μ*(x[1] + μ - 1)*(3*invr25 - 15*x[3]^2*invr27)];

    dG22dr          = @SVector [(1 - μ)*(x[1] + μ)*(3*invr15 - 15*x[2]^2*invr17) + 
                    μ*(x[1] + μ - 1)*(3*invr25 - 15*x[2]^2*invr27), 
                    (1 - μ)*x[2]*(9*invr15 - 15*x[2]^2*invr17) + 
                    μ*x[2]*(9*invr25 - 15*x[2]^2*invr27), 
                    (1 - μ)*x[3]*(3*invr15 - 15*x[2]^2*invr17) + 
                    μ*x[3]*(3*invr25 - 15*x[2]^2*invr27)];

    dG23dr          = @SVector [-15*(1 - μ)*(x[1] + μ)*x[2]*x[3]*invr17 - 
                    15*μ*(x[1] + μ - 1)*x[2]*x[3]*invr27, 
                    (1 - μ)*x[3]*(3*invr15 - 15*x[2]^2*invr17) + 
                    μ*x[3]*(3*invr25 - 15*x[2]^2*invr27), 
                    (1 - μ)*x[2]*(3*invr15 - 15*x[3]^2*invr17) + 
                    μ*x[2]*(3*invr25 - 15*x[3]^2*invr27)];

    dG33dr          = @SVector [(1 - μ)*(x[1] + μ)*(3*invr15 - 15*x[3]^2*invr17) + 
                    μ*(x[1] + μ - 1)*(3*invr25 - 15*x[3]^2*invr27), 
                    (1 - μ)*x[2]*(3*invr15 - 15*x[3]^2*invr17) + 
                    μ*x[2]*(3*invr25 - 15*x[3]^2*invr27), 
                    (1 - μ)*x[3]*(9*invr15 - 15*x[3]^2*invr17) + 
                    μ*x[3]*(9*invr25 - 15*x[3]^2*invr27)];  

    # Fill Jacobian
    jac[8,   1] = -dG11dr[1]*u[11] - dG12dr[1]*x[12] - dG13dr[1]*x[13]
    jac[8,   2] = -dG11dr[2]*u[11] - dG12dr[2]*x[12] - dG13dr[2]*x[13]
    jac[8,   3] = -dG11dr[3]*u[11] - dG12dr[3]*x[12] - dG13dr[3]*x[13]
    jac[9,   1] = -dG12dr[1]*u[11] - dG22dr[1]*x[12] - dG23dr[1]*x[13]
    jac[9,   2] = -dG12dr[2]*u[11] - dG22dr[2]*x[12] - dG23dr[2]*x[13]
    jac[9,   3] = -dG12dr[3]*u[11] - dG22dr[3]*x[12] - dG23dr[3]*x[13]
    jac[10,  1] = -dG13dr[1]*u[11] - dG23dr[1]*x[12] - dG33dr[1]*x[13]
    jac[10,  2] = -dG13dr[2]*u[11] - dG23dr[2]*x[12] - dG33dr[2]*x[13]
    jac[10,  3] = -dG13dr[3]*u[11] - dG23dr[3]*x[12] - dG33dr[3]*x[13]
    jac[1,   4] = 1.0
    jac[2,   5] = 1.0
    jac[3,   6] = 1.0
    jac[4,   1] = GVec[1]
...
function compute_G1!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ)
    @inbounds begin
        temp = γ*tMaxSc / (λv*x[7]*x[7]) + cSc*tMaxSc / (2.0*ϵ*x[7]^3)
        jac[4, 7] = temp*u[11]
        jac[5, 7] = temp*u[12]
        jac[6, 7] = temp*u[13]
    end
...
function compute_G7!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ)
    @inbounds jac[14,7] = 2.0*λv*γ*tMaxSc / (x[7]^3) + λv^2*cSc*tMaxSc / (2.0*ϵ*u[7]^4)
    return nothing
end
