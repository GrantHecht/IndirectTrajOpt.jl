...
function cr3bpEOMJac!(jac, u, p, GVec, homotopyFlag::MEMF)
    ...
    if p.utype != 1
        ...
        compute_G1!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G2!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G3!(jac, u, λv, tMaxSc, p.ϵ)
        compute_G4!(jac, u, λv, tMaxSc, p.ϵ)
        compute_G5!(jac, u, λv, tMaxSc, p.ϵ)
        compute_G6!(jac, p.ϵ, tMaxSc, cSc)
        compute_G7!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G8!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G9!(jac, u, λv, tMaxSc, p.ϵ)
    end
    return nothing
end

function cr3bpEOMJac!(jac, u, p, GVec, homotopyFlag::HypTanMF)
    ...
    if p.ϵ == 0.0
        ...
        compute_G1!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G2!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G3!(jac, u, λv, tMaxSc, cSc, p.ϵ)
        compute_G4!(jac, u, λv, tMaxSc, cSc, p.ϵ)
        compute_G5!(jac, u, λv, tMaxSc, cSc, p.ϵ)
        compute_G6!(jac, u, λv, tMaxSc, cSc, p.ϵ)
        compute_G7!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G8!(jac, u, λv, tMaxSc, cSc, p.ϵ, γ, homotopyFlag)
        compute_G9!(jac, u, λv, tMaxSc, cSc, p.ϵ)
    end
    return nothing
end


function compute_G1!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::MEMF)
    @inbounds begin
...
function compute_G1!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::HypTanMF)
    @inbounds begin
        S       = computeS(u, λv, cSc)
        temp    = γ*tMaxSc / (λv*u[7]*u[7]) - cSc*tMaxSc*(tanh(S/ϵ)^2 - 1)/(2*ϵ*u[7]^3)
        jac[4, 7] = temp*u[11]
        jac[5, 7] = temp*u[12]
        jac[6, 7] = temp*u[13]
    end
...
function compute_G2!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::MEMF)
    @inbounds begin
        λvInv   = 1.0 / λv
        λvInv2  = λvInv*λvInv
        λvInv3  = λvInv2*λvInv
...
function compute_G2!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::HypTanMF)
    @inbounds begin
        S       = computeS(u, λv, cSc)
        λvInv   = 1.0 / λv
        λvInv2  = λvInv*λvInv
        λvInv3  = λvInv2*λvInv
        temp1   = -γ*tMaxSc / u[7]
        temp2   = cSc*tMaxSc*(tanh(S/ϵ)^2 - 1) / (2.0*ϵ*u[7]^2)
        jac[4, 11] = temp1*(λvInv - u[11]*u[11]*λvInv3) + temp2*u[11]*u[11]*λvInv2
        jac[5, 12] = temp1*(λvInv - u[12]*u[12]*λvInv3) + temp2*u[12]*u[12]*λvInv2
        jac[6, 13] = temp1*(λvInv - u[13]*u[13]*λvInv3) + temp2*u[13]*u[13]*λvInv2
        jac[4, 12] = -temp1*u[11]*u[12]*λvInv3 + temp2*u[11]*u[12]*λvInv2 
...
function compute_G3!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ)
    @inbounds begin
        S           = computeS(u, λv, cSc)
        temp        = tMaxSc*(tanh(S/ϵ)^2 - 1.0) / (2.0*ϵ*λv*u[7])
        jac[4, 14]    = temp*u[11]
        jac[5, 14]    = temp*u[12]
        jac[6, 14]    = temp*u[13]
    end
...
function compute_G4!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ)
    S = computeS(u, λv, cSc)
    @inbounds jac[7,7] = -λv*tMaxSc*(tanh(S/ϵ)^2 - 1.0) / (2.0*ϵ*u[7]^2)
    return nothing
end
...
function compute_G5!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ)
    @inbounds begin
        S       = computeS(u, λv, cSc)
        temp    = tMaxSc*(tanh(S/ϵ)^2 - 1.0) / (2.0*ϵ*λv*u[7])
        jac[7, 11] = temp*u[11]
...
function compute_G6!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ)
    S = computeS(u, λv, cSc)
    @inbounds jac[7,14] = tMaxSc*(tanh(S/ϵ)^2 - 1.0) / (2.0*ϵ*cSc)
    return nothing
end
...
function compute_G7!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::MEMF)
    @inbounds jac[14,7] = 2.0*λv*γ*tMaxSc / (u[7]^3) + λv^2*cSc*tMaxSc / (2.0*ϵ*u[7]^4)
    return nothing
end

function compute_G7!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::HypTanMF)
    S = computeS(u, λv, cSc)
    @inbounds jac[14,7] = 2.0*λv*γ*tMaxSc / (u[7]^3) - λv^2*cSc*tMaxSc*(tanh(S/ϵ)^2 - 1.0) / (2.0*ϵ*u[7]^4)
    return nothing
end
...
function compute_G8!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::MEMF)
    @inbounds begin
...
function compute_G8!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ, γ, homotopyFlag::HypTanMF)
    @inbounds begin
        S       = computeS(u, λv, cSc)
        temp    = -γ*tMaxSc / (λv*u[7]^2) + cSc*tMaxSc*(tanh(S/ϵ)^2 - 1.0) / (2.0*ϵ*u[7]^3)
        jac[14, 11] = temp*u[11]
        jac[14, 12] = temp*u[12]
        jac[14, 13] = temp*u[13]
    end
...
function compute_G9!(jac::AbstractMatrix, u::AbstractVector, λv, tMaxSc, cSc, ϵ)
    S = computeS(u, λv, cSc)
    @inbounds jac[14,14] = λv*tMaxSc*(tanh(S/ϵ)^2 - 1.0) / (2.0*ϵ*u[7]^2)
    return nothing
end
